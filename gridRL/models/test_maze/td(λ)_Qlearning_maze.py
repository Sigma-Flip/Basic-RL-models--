from gridRL.env.maze_env import Mazeenv
...
class Agent():
    ...
    def update(self, state, action, reward, state_):
        ...
        next_q_values = self.get_q_values(state_)

        next_q_max = np.amax(next_q_values)
        delta = reward + self.gamma * next_q_max
...
if __name__ == '__main__':
    ...
    for game in range(n_games):
        ...
        while True:
            ...
            state_,reward, done = env.step(state, action, agent.memory)
            ...
            if done:
                if game == n_games-1:
                    ...
                    visualize(agent.memory, 'td(λ)_Qlearning', 'maze', n_games, total_reward)
